...
- [ ] 2. #276 core: `AccessSimulation.TeamKey`; filter applied only for the matching selected team in both claim paths. Tests first: applied on own team, ignored on another team, ignored without a team key, forged team key never adds.
- [ ] 3. #276 selection: `TeamStateService` keeps the simulated team selected when claims were issued for it; no cookie rewrite. Tests on the extracted decision (pure function beside `TeamSelectionResolver`).
