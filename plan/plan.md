...
- [x] **11. Docs.** Both surfaces reviewed.
  - **`docs/articles/implementation-guide.md`** gains a new section, *"Reading a team is decided the same way
    as changing one"*, next to the existing consent material: which callers reach a team's reads, that
    suspension withdraws the grant, and a "Changed in 3.18" note naming all three old behaviours and the
    consumer workaround that can now go.
  - **`README.md` needs no change, and that is the finding.** It already says a suspended member "hold[s] no
    scopes in it, so every scoped operation refuses", and already describes consent granting access to a
    non-member. The prose was right the whole time; the code was what disagreed with it. Nothing to correct —
    but worth recording, because "docs already claim the fixed behaviour" is why nobody caught this by reading.
  - `docs/api/*.yml` is DocFX-generated and regenerated by the `docs` job in CI, so the changed XML comments
    need no manual step.
