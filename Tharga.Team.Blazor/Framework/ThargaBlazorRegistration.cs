...
public static class ThargaBlazorRegistration
{
    ...
    public static void AddThargaTeamBlazor(this IServiceCollection services, Action<ThargaBlazorOptions> options = null, IConfiguration configuration = null)
    {
        ...
        if (o._teamService != null)
        {
            ...
            // them via ConfigureSystemRoles or a system API key. SystemScopeRegistry.Register skips a name
            // already present, so these need no per-line guard -- the one that used to sit here was
            // forgotten once already (Tharga/Team#237).
    ...
    private static void DecorateWithAuthorization(IServiceCollection services, TeamLifecycleOptions lifecycle)
    {
        ...
        services.AddScoped<ITeamService>(sp =>
        {
            ...
            var consentOptions = sp.GetService<IOptions<ConsentOptions>>()?.Value;
            return new AuthorizationTeamServiceDecorator(inner, authorizer, lifecycle, scopeRegistry, tenantRoleRegistry, dynamicRoleOptions?.ManageScope, purgeCascade, consentOptions);
        });
    }
